#include<stdio.h>
#include <math.h>
//Homework 1 method 1 - use machar() which is defined.
...
double get_epsdouble() {
	double doubleMachineAcc = 1;
	int n = 0;

	while ((double)(1 + doubleMachineAcc / 2.0) != 1) {
		doubleMachineAcc /= 2;
		n++;
	}
	return doubleMachineAcc;
}

float get_epsfloat() {
	float floatMachineAcc = 1;
	int n = 0;

	while ((float)(1 + floatMachineAcc / 2.0) != 1) {
		floatMachineAcc /= 2;
		n++;
	}
	return floatMachineAcc;
}

void main() {
	int ibeta, iexp, irnd, it, machep, maxexp, minexp, negep, ngrd;
	double eps1, epsneg1, xmax1, xmin1;

	float eps2, epsneg2, xmax2, xmin2;
	...
	printf("Machine accuracy of double by machar! = %12.6g\n", eps1);
	printf("Machine accuracy of float by machar! = %12.6g\n", eps2);

	printf("Machine accuracy of double by get_eps = %12.6g\n", get_epsdouble());
	printf("Machine accuracy of double by get_eps = %12.6g\n", get_epsfloat());
}
