...
#define MAXIT 30
float muller(float (*func)(float), float xx1, float xx2, float xacc) {
	//교과서 185쪽 muller method 계산 참고
	float root;
	float h0, h1, a, b, c, x3;
	float e1, e0;
	float x0 = xx1;
	float x1 = xx2;
	float x2 = (xx1 + xx2) / 2;
	...
	for (int i = 0; i < MAXIT; i++) {
		...
		h0 = x1 - x0;
		h1 = x2 - x1;
		e0 = (func(x1) - func(x0)) / (x1 - x0);
		e1 = (func(x2) - func(x1)) / (x2 - x1);
		a = (e1 - e0) / (h0 + h1);
		b = (a * h1) + e1;
		c = func(x2);

		float square_root = sqrt(b * b - 4 * a * c);  //b^2 - 4ac
		if (abs(b + square_root) > abs(b - square_root)) //positive
			x3 = x2 + ((-2 * c) / (b + square_root));
		else //negative;
			x3 = x2 + ((-2 * c) / (b - square_root));
		if (isnan(x3))
			break;
...
void main() {
	...
	printf("\n+) Muller's Method\n\n");
	cal1(muller, bessj0, x1, x2, xxacc, iteration);
}